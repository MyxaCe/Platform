...
## Вход и аккаунт (2026-07-22)

Терминал — **залогиненный продукт**: авторизация сессионной cookie (ADR-018), тот же аккаунт, что на
сайте и в кабинете. Демо-селектор Alice/Bob и `Bearer` из UI убраны. На старте `GET /auth/me`: не
вошёл → редирект на `accounts.<домен>`; вошёл → почта в шапке, ссылка в кабинет, кнопка выхода.
Свежий пользователь получает $100k бумажных денег ([[broker]]). Сделка, открытая в терминале, видна
в кабинете — один счёт.
