...
- **Стакан (order book)** в левой колонке — реальная глубина Binance (`/depth`, опрос ~700мс).
  ...
  Три режима показа (обе стороны / только покупки / только продажи) и **шаг группировки цен** —
  тик инструмента ×1/×10/×100 (для BTC 0.01/0.10/1.00, для монеты с одним знаком — 0.1/1/10).
